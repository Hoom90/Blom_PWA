...
import Step1 from '../components/addPlantView-GetPicture.vue'
...
<template>
    ...
    <div class="h-[100svh] max-w-[400px] w-full" @scroll="handleNavigationHeight">
        <Navigation :name="flower.name" :level="stage" @update:level="value => stage = value"></Navigation>
        <div class="flex flex-col items-center justify-center">
            <div v-if="stage == 1" class="mx-auto text-center mt-[150px]">به سه تا عکس از گلت نیاز داریم</div>
            <span v-if="stage == 2" class="mx-auto text-center mt-[150px]">سوالات زیر را تکمیل کنید </span>
        </div>

        <div class="flex flex-col justify-center max-w-[400px] w-full">

            <Step1 v-if="stage == 1"></Step1>
            <!-- stage 1 -->
            <!-- <div v-if="stage == 1" class="max-w-[400px] w-full flex flex-col gap-2 p-[20px]">

                <div class='flex flex-col gap-3 mx-auto w-full'>
                    <div class="flex flex-col gap-3 jusctify-center items-center">
                        <div class="max-w-[150px]" @click="openFileInput0">
                            <div class="relative flex justify-center p-[5px] items-center shadow-md cursor-pointer rounded">
                                <img v-if="userImages[0]" :src="userImages[0]"
                                    class="object-cover aspect-square w-[150px] rounded bg-white">
                                <div v-else
                                    class="aspect-square w-[150px] rounded bg-white flex items-center justify-center">
                    ...
                    <div class="flex gap-10 mx-auto">
                        <div class="flex flex-col gap-3 justify-center items-center">
                            <div class="max-w-[100px]" @click="openFileInput1">
                                <div
                                    class="relative flex justify-center p-[5px] items-center shadow-md cursor-pointer rounded">
                                    <img v-if="userImages[1]" :src="userImages[1]"
                                        class="object-cover aspect-square w-[200px] rounded bg-white">
                                    <div v-else
                                        class="aspect-square w-[200px] rounded bg-white flex items-center justify-center">
                                        <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd"
                                                d="M7.59843 4.48666C7.86525 3.17678 9.03088 2.25 10.3663 2.25H13.6337C14.9691 2.25 16.1347 3.17678 16.4016 4.48666C16.4632 4.78904 16.7371 5.01086 17.022 5.01086H17.0384L17.0548 5.01157C18.4582 5.07294 19.5362 5.24517 20.4362 5.83558C21.0032 6.20757 21.4909 6.68617 21.871 7.24464C22.3439 7.93947 22.5524 8.73694 22.6524 9.70145C22.75 10.6438 22.75 11.825 22.75 13.3211V13.4062C22.75 14.9023 22.75 16.0835 22.6524 17.0258C22.5524 17.9903 22.3439 18.7878 21.871 19.4826C21.4909 20.0411 21.0032 20.5197 20.4362 20.8917C19.7327 21.3532 18.9262 21.5567 17.948 21.6544C16.9903 21.75 15.789 21.75 14.2634 21.75H9.73657C8.21098 21.75 7.00967 21.75 6.05196 21.6544C5.07379 21.5567 4.26731 21.3532 3.56385 20.8917C2.99682 20.5197 2.50905 20.0411 2.12899 19.4826C1.65612 18.7878 1.44756 17.9903 1.34762 17.0258C1.24998 16.0835 1.24999 14.9023 1.25 13.4062V13.3211C1.24999 11.825 1.24998 10.6438 1.34762 9.70145C1.44756 8.73694 1.65612 7.93947 2.12899 7.24464C2.50905 6.68617 2.99682 6.20757 3.56385 5.83558C4.46383 5.24517 5.5418 5.07294 6.94523 5.01157L6.96161 5.01086H6.978C7.26288 5.01086 7.53683 4.78905 7.59843 4.48666ZM10.3663 3.75C9.72522 3.75 9.18905 4.19299 9.06824 4.78607C8.87258 5.74659 8.021 6.50186 6.99633 6.51078C5.64772 6.57069 4.92536 6.73636 4.38664 7.08978C3.98309 7.35452 3.63752 7.6941 3.36906 8.08857C3.09291 8.49435 2.92696 9.01325 2.83963 9.85604C2.75094 10.7121 2.75 11.8156 2.75 13.3636C2.75 14.9117 2.75094 16.0152 2.83963 16.8712C2.92696 17.714 3.09291 18.2329 3.36906 18.6387C3.63752 19.0332 3.98309 19.3728 4.38664 19.6375C4.80417 19.9114 5.33844 20.0756 6.20104 20.1618C7.07549 20.2491 8.20193 20.25 9.77778 20.25H14.2222C15.7981 20.25 16.9245 20.2491 17.799 20.1618C18.6616 20.0756 19.1958 19.9114 19.6134 19.6375C20.0169 19.3728 20.3625 19.0332 20.6309 18.6387C20.9071 18.2329 21.073 17.714 21.1604 16.8712C21.2491 16.0152 21.25 14.9117 21.25 13.3636C21.25 11.8156 21.2491 10.7121 21.1604 9.85604C21.073 9.01325 20.9071 8.49435 20.6309 8.08857C20.3625 7.6941 20.0169 7.35452 19.6134 7.08978C19.0746 6.73636 18.3523 6.57069 17.0037 6.51078C15.979 6.50186 15.1274 5.74659 14.9318 4.78607C14.8109 4.19299 14.2748 3.75 13.6337 3.75H10.3663ZM12 9.25C12.4142 9.25 12.75 9.58579 12.75 10V12.25H15C15.4142 12.25 15.75 12.5858 15.75 13C15.75 13.4142 15.4142 13.75 15 13.75H12.75V16C12.75 16.4142 12.4142 16.75 12 16.75C11.5858 16.75 11.25 16.4142 11.25 16V13.75H9C8.58579 13.75 8.25 13.4142 8.25 13C8.25 12.5858 8.58579 12.25 9 12.25H11.25V10C11.25 9.58579 11.5858 9.25 12 9.25ZM17.25 10C17.25 9.58579 17.5858 9.25 18 9.25H19C19.4142 9.25 19.75 9.58579 19.75 10C19.75 10.4142 19.4142 10.75 19 10.75H18C17.5858 10.75 17.25 10.4142 17.25 10Z"
                        ...
                        <div class="flex flex-col gap-3 jusctify-center items-center">
                            <div class="max-w-[100px]" @click="openFileInput2">
                                <div
                                    class="relative flex justify-center p-[5px] items-center shadow-md cursor-pointer rounded">
                                    <img v-if="userImages[2]" :src="userImages[2]"
                                        class="object-cover aspect-square w-[200px] rounded bg-white">
                                    <div v-else
                                        class="aspect-square w-[200px] rounded bg-white flex items-center justify-center">
                                        <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none"
                ...
                <div class="flex flex-col justify-center items-center gap-[10px]">
                    ...
                    <div class="shadow-[0_4px_10px_0_#c9c9c9] rounded-md w-full  p-[15px] bg-white">
                        ...
                        <div class="flex justify-center items-center gap-[10px]" id="lightData" @click="setLight">
                            ...
                            <button @click="setLight(1)" ref="directLight"
                                class="relative w-1/3 border flex items-center justify-center h-[60px] p-[10px] text-[#adadad] bg-[#f6f6f6] rounded-md">
                                <div class="flex flex-col items-center justify-center">
                                ...
                                <div v-if="directLightCheck"
                                    class="absolute -bottom-2 -right-2 bg-[#49b779] rounded-full p-[2px] justify-center items-center text-white">
                            ...
                            <button @click="setLight(2)" ref="semiLight"
                                class="relative w-1/3 border flex items-center justify-center h-[60px] p-[10px] text-[#adadad] bg-[#f6f6f6] rounded-md">
                                <div class="flex flex-col items-center justify-center">
                            ...
                            <button @click="setLight(3)" ref="shadow"
                                class="relative w-1/3 border flex items-center justify-center h-[60px] p-[10px] text-[#adadad] bg-[#f6f6f6] rounded-md">
                                <div class="flex flex-col items-center justify-center">
                        ...
                        <div class="flex mt-[10px] gap-1 text-[#AC0202]" v-if="LightError">
                            <i fill="currentcolor">
                                <svg width="10" height="10" viewBox="0 0 21 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M10.1235 0C4.53531 0 0 4.48 0 10C0 15.52 4.53531 20 10.1235 20C15.7116 20 20.2469 15.52 20.2469 10C20.2469 4.48 15.7116 0 10.1235 0ZM11.1358 15H9.11111V9H11.1358V15ZM11.1358 7H9.11111V5H11.1358V7Z"
                                        fill="currentcolor" />
                    ...
                    <div class="shadow-[0_4px_10px_0_#c9c9c9] rounded-md w-full p-[15px] bg-white">
                        ...
                        <div class="flex gap-[5px] items-center w-full">
                            <select ref="tempSelect" class="w-full p-[5px] border outline-none transition-all duration-200"
                                @input="setTemp">
                    ...
                    <div class="shadow-[0_4px_10px_0_#c9c9c9] rounded-md w-full p-[15px] bg-white">
                        ...
                        <div class='grid grid-cols-3 gap-1 justify-center items-center'>
                            <button v-for="(plantStatus, index) in plantStatuses" @click="setSymptoms(index)"
                                :name="'symptoms' + index"
                                class="col-1 border h-[40px] flex items-center justify-center p-[5px] text-[12px] text-[#adadad] bg-[#f6f6f6] rounded-md">{{
                                    plantStatus }}
                            </button>
                        </div>
                    ...
                    <div class="shadow-[0_4px_10px_0_#c9c9c9] rounded-md w-full p-[15px] bg-white">
                        ...
                        <div class='flex justify-center items-center'>
                            <textarea @input="setDescription" v-model='description' cols="30" rows="3"
                                class="outline-none input resize-none"
                                placeholder="علائم بیماری را کامل بنویسید"></textarea>
                        </div>
                    </div>
            ...
            <div v-if="stage == 3" class="max-w-[400px] w-full flex flex-col gap-2 p-[20px]">
                ...
                <div class="flex flex-col gap-[10px] text-[12px] justify-center items-center">
                    <span class='font-semibold'>اطلاعات دریافتی با دقت<span class='text-[#49b779]'> 80٪</span>راه درمان را
                        ارائه می دهد</span>
                    <!-- <span class="text-[10px]">برای دریافت نسخه درمان  ۱۰۰٪ میتوانیداطلاعات گیاه خود راتکمیل کنید</span> -->
                </div>

                <div v-if="compeleteInfo"
                    class="shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md flex flex-col gap-[10px] bg-white">
                    ...
                    <div class='flex flex-col gap-[5px] shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md bg-white'>
                        ...
                        <div class="flex flex-wrap justify-center gap-[5px] text-[14px]">
                            <button @click="setWaterGlass(1)" ref="halfGlass"
                                class='flex flex-col justify-center items-center p-2 bg-[#f6f6f6] rounded-md border'>
                                <i>
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="24px"
                                        height="32px" viewBox="0 0 24 32" enable-background="new 0 0 24 32"
                                        xml:space="preserve">
                                        <image id="image0" width="24" height="32" x="0" y="0" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAgCAMAAAA/gEgKAAAABGdBTUEAALGPC/xhBQAAACBjSFJN
                            ...
                            <button @click="setWaterGlass(2)" ref="oneGlass"
                                class='flex flex-col justify-center items-center p-2 bg-[#f6f6f6] rounded-md border'>
                                <i>
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="24px"
                            ...
                            <button @click="setWaterGlass(4)" ref="twoGlass"
                                class='flex flex-col justify-center items-center p-2 bg-[#f6f6f6] rounded-md border'>
                                <i>
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="50px"
                                        height="32px" viewBox="0 0 50 32" enable-background="new 0 0 50 32"
                                        xml:space="preserve">
                                        <image id="image0" width="50" height="32" x="0" y="0" href="data:image/png;base64,
                    ...
                    <div class='flex flex-col gap-[5px] shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md bg-white'>
                        ...
                        <div class="flex flex-wrap justify-center gap-[5px]">
                            <button @click="setDrainage(true)" ref="drain"
                                class='flex flex-col justify-center items-center p-2 bg-[#f6f6f6] rounded-md border'>
                                <i>
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="43px"
                                        height="38px" viewBox="0 0 43 38" enable-background="new 0 0 43 38"
                                        xml:space="preserve">
                                        <image id="image0" width="43" height="38" x="0" y="0" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACsAAAAmCAMAAABqHZIsAAAABGdBTUEAALGPC/xhBQAAACBjSFJN
                            ...
                            <button @click="setDrainage(false)" ref="non_drain"
                                class='flex flex-col justify-center items-center p-2 bg-[#f6f6f6] rounded-md border'>
                                <i>
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="43px"
                                        height="30px" viewBox="0 0 43 30" enable-background="new 0 0 43 30"
                                        xml:space="preserve">
                                        <image id="image0" width="43" height="30" x="0" y="0" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACsAAAAeCAMAAACCNBfsAAAABGdBTUEAALGPC/xhBQAAACBjSFJN
                    ...
                    <div class='flex flex-col gap-[5px] shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md bg-white'>
                        <div class="flex items-center gap-[5px]">
                            <i>
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M5.25 3.1875H12.75V2.0625H5.25V3.1875ZM15.1875 5.625V13.5H16.3125V5.625H15.1875ZM12.75 15.9375H5.25V17.0625H12.75V15.9375ZM2.8125 13.5V5.625H1.6875V13.5H2.8125ZM5.25 15.9375C4.60353 15.9375 3.98355 15.6807 3.52643 15.2236C3.06931 14.7665 2.8125 14.1465 2.8125 13.5H1.6875C1.6875 14.4448 2.06283 15.351 2.73093 16.0191C3.39903 16.6872 4.30517 17.0625 5.25 17.0625V15.9375ZM15.1875 13.5C15.1875 14.1465 14.9307 14.7665 14.4736 15.2236C14.0165 15.6807 13.3965 15.9375 12.75 15.9375V17.0625C13.6948 17.0625 14.601 16.6872 15.2691 16.0191C15.9372 15.351 16.3125 14.4448 16.3125 13.5H15.1875ZM12.75 3.1875C13.3965 3.1875 14.0165 3.44431 14.4736 3.90143C14.9307 4.35855 15.1875 4.97853 15.1875 5.625H16.3125C16.3125 4.68017 15.9372 3.77403 15.2691 3.10593C14.601 2.43783 13.6948 2.0625 12.75 2.0625V3.1875ZM5.25 2.0625C4.30517 2.0625 3.39903 2.43783 2.73093 3.10593C2.06283 3.77403 1.6875 4.68017 1.6875 5.625H2.8125C2.8125 4.97853 3.06931 4.35855 3.52643 3.90143C3.98355 3.44431 4.60353 3.1875 5.25 3.1875V2.0625Z"
                                        fill="#49B779" />
                                    <path
                                        d="M2.25 6.1875C2.10082 6.1875 1.95774 6.24676 1.85225 6.35225C1.74676 6.45774 1.6875 6.60082 1.6875 6.75C1.6875 6.89918 1.74676 7.04226 1.85225 7.14775C1.95774 7.25324 2.10082 7.3125 2.25 7.3125V6.1875ZM15.75 7.3125C15.8992 7.3125 16.0423 7.25324 16.1477 7.14775C16.2532 7.04226 16.3125 6.89918 16.3125 6.75C16.3125 6.60082 16.2532 6.45774 16.1477 6.35225C16.0423 6.24676 15.8992 6.1875 15.75 6.1875V7.3125ZM2.25 7.3125H15.75V6.1875H2.25V7.3125Z"
                                        fill="#49B779" />
                                    <path
                                        d="M6.5625 1.5C6.5625 1.35082 6.50324 1.20774 6.39775 1.10225C6.29226 0.996763 6.14918 0.9375 6 0.9375C5.85082 0.9375 5.70774 0.996763 5.60225 1.10225C5.49676 1.20774 5.4375 1.35082 5.4375 1.5H6.5625ZM5.4375 3.75C5.4375 3.89918 5.49676 4.04226 5.60225 4.14775C5.70774 4.25324 5.85082 4.3125 6 4.3125C6.14918 4.3125 6.29226 4.25324 6.39775 4.14775C6.50324 4.04226 6.5625 3.89918 6.5625 3.75H5.4375ZM5.4375 1.5V3.75H6.5625V1.5H5.4375Z"
                                        fill="#49B779" />
                                    <path
                                        d="M12.5625 1.5C12.5625 1.35082 12.5032 1.20774 12.3977 1.10225C12.2923 0.996763 12.1492 0.9375 12 0.9375C11.8508 0.9375 11.7077 0.996763 11.6023 1.10225C11.4968 1.20774 11.4375 1.35082 11.4375 1.5H12.5625ZM11.4375 3.75C11.4375 3.89918 11.4968 4.04226 11.6023 4.14775C11.7077 4.25324 11.8508 4.3125 12 4.3125C12.1492 4.3125 12.2923 4.25324 12.3977 4.14775C12.5032 4.04226 12.5625 3.89918 12.5625 3.75H11.4375ZM11.4375 1.5V3.75H12.5625V1.5H11.4375Z"
                                        fill="#49B779" />
                                    <path
                        ...
                        <div class='flex gap-[10px] justify-center'>
                            <div class="w-1/3 flex flex-col justify-center items-center">
                                <button @click="incrementSickNumber"
                                    class="bg-[#49b779] text-white px-3 rounded w-[60px] justify-center flex">
                                ...
                                <input type="text" id="sickNumber" class="w-full bg-transparent text-center"
                                    v-model="sickNumber" disabled />
                                <button @click="decrementSickNumber"
                                    class="bg-[#49b779] text-white px-3 rounded w-[60px] justify-center flex">
                                    <svg fill="currentcolor" width="20" height="20" viewBox="0 0 1920 1920"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                            ...
                            <div class="w-1/3 flex flex-col justify-center items-center">
                                <button @click="incrementSickNumberType"
                                    class="bg-[#49b779] text-white px-3 rounded w-[60px] justify-center flex">
                                    <svg fill="currentcolor" width="20" height="20" viewBox="0 0 1920 1920"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                <input type="text" id="sickNumberType" class="w-full bg-transparent text-center"
                                    v-model="sickNumberTypeName" disabled />
                                <button @click="decrementSickNumberType"
                                    class="bg-[#49b779] text-white px-3 rounded w-[60px] justify-center flex">
                                    <svg fill="currentcolor" width="20" height="20" viewBox="0 0 1920 1920"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                    ...
                    <div class='flex gap-[5px] shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md bg-white'>
                        <div class="flex items-center gap-[5px]">
                            <i>
                                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="17px" height="21px"
                                    viewBox="0 0 17 21" enable-background="new 0 0 17 21" xml:space="preserve">
                                    <image id="image0" width="17" height="21" x="0" y="0" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABEAAAAVCAMAAACXIvXeAAAABGdBTUEAALGPC/xhBQAAACBjSFJN
                    ...
                    <div class='flex flex-col gap-[5px] shadow-[0_0_5px_0_#c9c9c9] p-[10px] rounded-md bg-white'>
                        <div class="flex items-center gap-[5px]">
                            <i>
                                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="14px" height="18px"
                                    viewBox="0 0 14 18" enable-background="new 0 0 14 18" xml:space="preserve">
                                    <image id="image0" width="14" height="18" x="0" y="0" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAASCAMAAABcgh8DAAAABGdBTUEAALGPC/xhBQAAACBjSFJN
                        ...
                        <div class='flex gap-[5px]'>
                            <textarea cols="30" rows="10" class="outline-none input resize-none"
                                placeholder='نوع کود یا سم های در حال استفاده و نحوه مصرف را کامل توضیح دهید'></textarea>
                        </div>
                    </div>
            ...
            <!-- buttons -->
            <!-- <div class="flex justify-center items-center">
                <button v-if="stage == 1" @click="saveForm"
                    class="text-white bg-[#49B779] font-semibold rounded-md max-w-[200px] w-full px-2 p-1 shadow-[0_4px_4px_0_#c9c9c9]">
                    مرحله بعدی
                </button>
                <button v-if="stage == 2" @click="saveForm"
                    class="text-white bg-[#49B779] font-semibold rounded-md max-w-[200px] w-full px-2 p-1 shadow-[0_4px_4px_0_#c9c9c9]">
                    ثبت اطلاعات
                </button>
                <div v-if="stage == 3" class="flex gap-[10px]">
                    <button @click="saveForm"
                        class="text-white bg-[#49B779] font-semibold rounded-md p-1 px-3 shadow-[0_4px_4px_0_#c9c9c9]">
                        دریافت نسخه
                    </button>
                </div>
            </div> -->
    ...
    <div v-if="finishProcess" class="absolute top-0 left-0 w-full bg-[#fffe]">
        <div class="h-[90vh] flex flex-col gap-[10px] text-[#49b779] items-center justify-center px-[20px]">
            ...
            <RouterLink to="/garden" class="bg-gray-300 rounded text-white text-center w-[100px] px-2 p-1" disabled>دیدن
                وضعیت نسخه های من</RouterLink>
            <RouterLink to="/garden/plantlibrary" class="bg-[#49b779] rounded text-white text-center w-[100px] px-2 p-1">
                گیاه دیگر</RouterLink>
        </div>
    </div>
</template>
