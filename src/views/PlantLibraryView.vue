...
const searchWord = () => {
  ...
  ul = document.getElementById("plantlibrary");
...
<template>
    <div class="overflow-y-auto h-[100svh] bg-white relative">
        <div id="plantheader" class="max-w-[400px] flex flex-col gap-3 fixed w-full px-[20px] pt-[20px] pb-[10px] z-10 bg-white">
            <div>
                <div class="flex gap-[5px] w-full">
                    <div class="bg-white border-[#ededed] border rounded-md flex gap-1 justify-between p-2 w-full h-[40px]">
                        ...
                        <input ref="searchBox" type="text" class="bg-transparent w-full outline-none text-[#9c9c9c] placeholder-[#9c9c9c]" @input="searchBoxEffect" @keyup="searchWord" placeholder="گیاه رو اینجا بگرد...">
                    ...
                    <!-- <button class="border rounded-md p-2 h-[40px] flex items-center text-[#9c9c9c]" @click="sortTable">
            ...
            <p class="font-bold">گیاه خود را انتخاب کنید</p>
        </div>
        
        <div id="plantlibrary" class="mt-[100px] px-[20px] mb-[30px]">


            <RouterLink :to="'/addplant/'+flower.id" class="flex flex-col gap-[5px]" v-for="flower in flowers">
            ...
            <!-- <RouterLink v-else :to="'/addplant/'+flower.id" class="flex flex-col gap-[5px]" loading="lazy" v-for="flower in flowers.reverse()">
                <div class="flex items-center gap-[5px] py-2 border-b-[1px]">
                    <img :src="LIBRARY_URL+flower.image" class="aspect-square object-cover w-[50px] h-[50px] rounded-full" :alt="flower.name"/>
                    <span>{{ flower.name }}</span>
                </div>
